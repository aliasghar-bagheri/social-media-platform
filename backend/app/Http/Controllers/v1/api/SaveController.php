...
class SaveController extends Controller
{
    ...
    public function save(Request $request)
    {
        ...
        if ($accessToken) {
            ...
            $is_save = DB::table('save')->where("user_id", $userId)->where("post_id", $request->post_id)->exists();
            if ($is_save) {
                DB::table('save')->where("user_id", $userId)->where("post_id", $request->post_id)->delete();

                return response()->json([
                    'status' => 200,
                    "essage" => "unsave post successfully.",
                ], 200);
            } else {
